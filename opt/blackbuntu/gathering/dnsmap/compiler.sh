...
function set_banner
{
	...
	echo
	echo -e "${TEXT_VALID} [i] [Package]: dnsmap-compiler${TEXT_RESET}"
...
## ----------------------------
echo -e " Dnsmap is not yet compiled."
...
then
	...
	echo -e " Please wait while compiling Dnsmap ...${TEXT_RESET}"
	sleep 3s

	sudo rm -f /usr/bin/dnsmap
	cd /opt/blackbuntu/gathering/dnsmap/
	sudo make >/dev/null 2>&1
	sudo gcc -Wall dnsmap.c -o dnsmap >/dev/null 2>&1
	sudo ln -s /opt/blackbuntu/gathering/dnsmap/dnsmap /usr/bin/dnsmap

	echo -e " ${TEXT_VALID}[i]${TEXT_RESET} Dnsmap has been compiled successfully${TEXT_RESET}"
	echo -e " ${TEXT_VALID}[i]${TEXT_RESET} Close this terminal windows and re-open Dnsmap"
	echo -e "\n"
else
	echo
