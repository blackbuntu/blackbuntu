...
function set_banner
{
	...
	echo
	echo -e "${TEXT_VALID} [i] [Package]: slowhttptest${TEXT_RESET}"
...
## ----------------------------
echo -e " SlowHTTPTest is not yet compiled."
...
then
	...
	echo -e " Please wait while compiling SlowHTTPTest ...${TEXT_RESET}"
	sleep 5s

	sudo rm -f /usr/bin/slowhttptest
	cd /opt/blackbuntu/stress-testing/slowhttptest/
	...
	echo
	echo -e " ${TEXT_VALID}[i]${TEXT_RESET} SlowHTTPTest has been compiled successfully${TEXT_RESET}"
